...
export default function AuthClient() {
  ...
  const checkClientExists = async (phoneNumber: string) => {
    try {
      const response = await fetch(
        `${apiUrl}/auth/client/find-by-phone/+${phoneNumber}`
      );
  ...
  const verifyCodeAndProceedTG = async () => {
    ...
    try {
      const verifyResponse = await fetch(
        `${apiUrl}/telegram/verify-code/${code.code}`
      );
      ...
      if (
        ...
        const clearResponse = await fetch(
          `${apiUrl}/telegram/clear-code/${code.code}`,
          {
            method: "POST",
            headers: {
  ...
  const verifyCodeAndProceedWA = async () => {
    ...
    try {
      const verifyResponse = await fetch(
        `${apiUrl}/whatsapp/verify-code/${code.code}`
      );
      ...
      if (
        status === "success" &&
        phone.replace(/ /g, "") === phone.replace(/ /g, "").replace(/\D/g, "")
      ) {
        const clearResponse = await fetch(
    ...
    if (step < totalSteps) {
      if (step === 1 && isPhoneValid) {
        ...
        if (codeType == "TG" && clientExists) {
          let res = await fetch(
            `${apiUrl}/telegram/send_code/client/+${phone.replace(/\D/g, "")}`,
            {
              method: "POST",
            }
          );
          ...
          const sendResponse = await fetch(
            `${apiUrl}/whatsapp/send-code/${cleanPhone}`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
      ...
      case 2:
        return (
          <>
            ...
            <CodeEnter
              onSubmit={(code: string) => setCode(code)}
      ...
      case 3:
        return (
          <>
            ...
            <Congratulations
              name={{
                first_name: useData.name,
                surname: useData.last_name,
                patronymic: useData.sur_name,
              }}
            />
          </>
        );
    ...
    >
      ...
      >
        <Alert
          ...
          sx={{ width: "100%" }}
        >
          Клиент с таким номером телефона не найден. Пожалуйста,
          зарегистрируйтесь.
        </Alert>
      </Snackbar>
      ...
      >
        <Alert
          ...
          sx={{ width: "100%" }}
        >
          Неверный код подтверждения. Пожалуйста, проверьте код и попробуйте
          снова.
        </Alert>
      </Snackbar>
    </Box>
