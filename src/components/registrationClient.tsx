...
export default function RegistrationClient() {
  ...
  const verifyCodeAndProceedTg = async () => {
    ...
    try {
      const verifyResponse = await fetch(
        `${apiUrl}/telegram/verify-code/${formData.code.code}`
      );
      ...
      if (
        status === "success" &&
        phone.replace(/ /g, "") === formData.phone.replace(/ /g, "")
      ) {
        const clearResponse = await fetch(
          `${apiUrl}/telegram/clear-code/${formData.code.code}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const verifyCodeAndProceedWa = async () => {
    ...
    try {
      const verifyResponse = await fetch(
        `${apiUrl}/whatsapp/verify-code/${formData.code.code}`
      );
      ...
      if (
        status === "success" &&
        phone.replace(/ /g, "") ===
          formData.phone.replace(/ /g, "").replace(/\D/g, "")
      ) {
        const clearResponse = await fetch(
          `${apiUrl}/whatsapp/clear-code/${formData.code.code}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
